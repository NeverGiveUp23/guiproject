...
void WindowClass::DrawMenu()
{
    ...
    ImGui::Text("Current Directory: %s", currentDir.string().c_str()); // display current Directory -> convert to string before so all OS has the same behaivior
}
...
void WindowClass::DrawContent()
{
    for (const auto& entry : fs::directory_iterator(currentDir)) {
        ...
        if (ImGui::Selectable(entry_name.c_str(), is_selected)) // display just the last portion of the file path -> and just want to output a string
        {
            if (is_Dir) {
...
void WindowClass::DrawFilter()
{
    ...
    ImGui::Text("Filter ");
    ImGui::SameLine();
    ImGui::InputText("###Input Filter", ext_filter, sizeof(ext_filter)); // ### -> to not display that text. ext_filter an array of chars for the text and the size being no more than the sizeof ext_filter
    ...
    {
        ImGui::TextColored(ImVec4(1.0F, 2.4F, 0.0F, 2.4F), // -> display message in yellow colored text
                           "Too many characters");
        return;
    }
...
void WindowClass::renameFilePopup(){
    ...
    if (ImGui::BeginPopupModal("Rename File"))
    {
        ...
        ImGui::Text("New Name");
        ImGui::InputText("###newname", buffer_name, sizeof(buffer_name)); // giving it the size and variable to store name in
